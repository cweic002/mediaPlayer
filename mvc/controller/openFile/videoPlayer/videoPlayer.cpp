...
Controller::OpenFile::VideoPlayer::VideoPlayer(Model::VideoPlayer * modelVideoPlayer,char * src):
          ControllerCRTP(modelVideoPlayer),src(src){}

Controller::OpenFile::VideoPlayer::VideoPlayer(std::shared_ptr<Model::VideoPlayer> & modelVideoPlayer,char * src):
          ControllerCRTP(modelVideoPlayer),src(src){}

inline void Controller::OpenFile::VideoPlayer::open(char * src){
  if(this->model && model->videoPlayer){
    gst_element_set_state(model->videoPlayer, GST_STATE_NULL);
    if(src){
      g_object_set(model->src, "location",src, NULL);
    }else{
      g_object_set(model->src, "location",this->src, NULL);
    }
  }
}
