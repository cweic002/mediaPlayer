...
Controller::Mute::VideoPlayer::VideoPlayer(Model::VideoPlayer * modelVideoPlayer):
          ControllerCRTP(modelVideoPlayer){}

Controller::Mute::VideoPlayer::VideoPlayer(std::shared_ptr<Model::VideoPlayer> & modelVideoPlayer):
          ControllerCRTP(modelVideoPlayer){}

inline void Controller::Mute::VideoPlayer::setMute(bool mute){
  if(this->model && model->videoPlayer){
    if(model->audio){
      g_object_set(G_OBJECT(model->audio),"mute",mute, NULL);
    }
  }
}

inline bool Controller::Mute::VideoPlayer::getMute(){
  if(this->model && model->audio){
    gboolean mute;
    g_object_get(G_OBJECT(model->audio),"mute",&mute, NULL);
    return mute;
  }
  return true;
}
