...
Controller::Position::VideoPlayer::VideoPlayer(Model::VideoPlayer * modelVideoPlayer):
...
inline void Controller::Position::VideoPlayer::setStep(std::chrono::milliseconds step){
  if(this->model && model->audio){
    gint64 position;
    gst_element_query_position(model->videoPlayer,GST_FORMAT_TIME,&position);
    gint64 gst_step = (gint64)(step.count() * GST_MSECOND);
    gst_element_seek_simple(model->videoPlayer, GST_FORMAT_TIME,
              (GstSeekFlags)(GST_SEEK_FLAG_KEY_UNIT | GST_SEEK_FLAG_FLUSH),position+gst_step);
  }
}
