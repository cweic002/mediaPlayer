...
View::VideoPlayer::VideoPlayer(QWidget *parent,Model::VideoPlayer * modelVideoPlayer):QWidget(parent),
            ViewCRTP(modelVideoPlayer)
{
    this->setWindowHandle();
}

void View::VideoPlayer::setWindowHandle(){
    if(model){
        gst_video_overlay_set_window_handle(GST_VIDEO_OVERLAY(model->video),this->winId());
    }
}
